...
class Xing(BaseSite):
    ...
    async def login(self) -> bool:
        ...
        await self.delay()
        await self.safe_click('button[data-qa="consent-accept-btn"], button:has-text("Accept all")', 3000)

        await self.safe_fill('#login-form input[type="email"], input[name="email"], #email', email)
        await self.delay(0.3, 0.7)
        await self.safe_fill('input[type="password"], input[name="password"]', password)
        await self.delay(0.3, 0.7)
        await self.safe_click('button[type="submit"], button:has-text("Login")')
        await self.page.wait_for_load_state("domcontentloaded")
        await self.wait_for_captcha()

        success = "login" not in self.page.url.lower() and "xing.com" in self.page.url
        logger.info(f"xing login {'OK' if success else 'FAILED'}")
        return success
    ...
    async def search_jobs(self, keywords: list[str], location: str) -> list[dict]:
        ...
        for keyword in keywords[:5]:
            ...
            for _ in range(3):
                cards = await self.page.query_selector_all(
                    '[data-testid="job-list-item"], .job-listing-item, article[data-qa="job-card"]'
                )
                for card in cards:
                    try:
                        title_el = await card.query_selector(
                            '[data-qa="job-title"] a, h2 a, h3 a, .job-listing-title a'
                        )
                        ...
                        loc_el = await card.query_selector(
                            '[data-qa="location"], .location-name, .job-listing-location, '
                            '[class*="location"]'
                        )
        ...
        logger.info(f"xing: found {len(jobs)} jobs")
        return jobs
    ...
    async def apply_to_job(self, job: dict, cover_letter: str = "") -> bool:
        ...
        try:
            desc_el = await self.page.query_selector(
                '[data-qa="job-description"], .job-description, .description'
            )
            if desc_el:
        ...
        clicked = await self.safe_click(
            'a[data-qa="apply-button"], button:has-text("Jetzt bewerben"), '
            'a:has-text("Jetzt bewerben"), [data-testid="apply-button"]',
            5000
        )
        if not clicked:
            logger.warning(f"xing: no apply button for {job['title']}")
            return False
        ...
        if cover_letter:
            await self.safe_fill(
                'textarea[name*="coverLetter"], textarea[name*="motivation"], textarea[name*="message"]',
                cover_letter
            )
        ...
        logger.info(f"xing: {'applied' if submitted else 'failed'} -> {job['title']}")
        return submitted
